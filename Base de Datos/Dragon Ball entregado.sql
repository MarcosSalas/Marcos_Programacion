...
-- #################  Punto f y g .a)--
 create table Cantidad_por_Especie as 
...
select * from Cantidad_por_Especie;
drop table Cantidad_por_Especie;

-- #################  Punto g .b)
create view Datos_Personaje as
 ...
 select * from Datos_Personaje;

-- #################  Punto g .c)
create view Nombre_Personaje_Id_Familia as 
select personaje.nombre,familia.id_Familia from personaje inner join familia on personaje.id_Familia=familia.id_Familia where familia.Nombre_Madre like 'Desconocida';
 select * from Nombre_Personaje_Id_Familia;
-- drop view Nombre_Personaje_Id_Familia;


-- ################## Punto H 
create index idx_nombre_Lnacimiento on personaje (Nombre,Lugar_Nacimiento);
create index idx_Nombre_Padre_Madre on familia (Nombre_Padre,Nombre_Madre);

EXPLAIN SELECT Nombre, Lugar_Nacimiento FROM personaje;
EXPLAIN SELECT Nombre_Padre, Nombre_Madre FROM familia;

-- drop index idx_nombre_Lnacimiento on personaje;
-- drop index idx_Nombre_Padre_Madre on familia;

-- #################      Punto i

create role if not exists 'Dios_de_Todo','Dios_Destruccion';
grant all on dragonball_z.* to 'Dios_de_Todo';
grant select, delete on dragonball_z.* to 'Dios_Destruccion'; 
...
create user 'ShenLong'@'localhost' identified by 'admin';
GRANT INSERT, UPDATE ON  dragonball_z.* to 'ShenLong'@'localhost';

create user 'GranPatriarca'@'localhost' identified by 'admin';
GRANT SELECT, UPDATE ON  dragonball_z.* TO 'GranPatriarca'@'localhost';

create user 'Kaiossama'@'localhost' identified by 'admin';
GRANT SELECT ON  dragonball_z.* TO 'Kaiossama'@'localhost';

-- show grants for 'ShenLong'@'localhost';
  
  
  -- #################      Punto j
DELIMITER $$
CREATE PROCEDURE Poderes_por_Personaje(IN id int,out cant_Poderes int)
BEGIN
CASE id
    WHEN 1 then 
    set cant_Poderes= (select count(distinct poder.nombre) from poder inner join poder_personaje on poder.id_Poder=poder_personaje.id_Poder);    
    WHEN 2 then 
	set cant_Poderes= (select count(distinct poder.nombre) from poder inner join poder_personaje on poder.id_Poder=poder_personaje.id_Poder);    
	WHEN 3 then 
	set cant_Poderes= (select count(distinct poder.nombre) from poder inner join poder_personaje on poder.id_Poder=poder_personaje.id_Poder);    
...
END CASE;
END $$
call Poderes_por_Personaje (5, @cant_Poderes);
select @cant_Poderes;
